...
class InteropRunner:
    ...
    measurement_results: Dict[str, Dict[str, Dict[str, Any]]] = {}
    compliant = {}
    _implementations = {}
    _servers: List[str] = []
    _clients: List[str] = []
    _tests: List[Type[testcases.TestCase]] = []
    _measurements: List[Type[testcases.Measurement]] = []
    _output = ""
    _log_dir = ""
    _save_files = False
    ...
    def _run_test(
        ...
        print(f"Server: {server}. Client: {client}. Running test case: {testcase}")
        ...
        if status in (TestResult.FAILED, TestResult.SUCCEEDED):
            log_dir = f"{self._log_dir}/{server}_{client}/{testcase}"
